...
class TestAPIEndpoints:
    ...
    def test_institution_endpoints(self):
        ...
        # Update
        response = self.client.patch(f'/api/institutions/{inst_id}/', {
            'name': 'Updated Bank'
        })
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_account_endpoints(self):
        ...
        inst_response = self.client.post('/api/institutions/', {
            'name': 'Test Bank',
            'identifier': 'testbank'
        })
    ...
    def test_transaction_endpoints(self):
        ...
        inst_resp = self.client.post('/api/institutions/', {
            'name': 'Bank',
            'identifier': 'bank'
        })
        inst_id = inst_resp.data['id']
        
        acc_resp = self.client.post('/api/accounts/', {
            'institution': inst_id,
            'name': 'Checking',
            'account_number': '1234'
        })
